...
def extract_json(response):
    """Extracts the JSON part from the model output."""
    match = re.search(r'(\{.*?\})', response, re.DOTALL)
    if match:
        return match.group(1)  # Return just the JSON part
    return '{"total_carbohydrates": -1}'  # Fallback
...
def generate_response(model, tokenizer, prompt, max_new_tokens=50, temperature=0.1, top_p=0.1):
    ...
    json_response = extract_json(full_response)  # Extract just the JSON part
